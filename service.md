...
|---|---|---|---|
| 1 | LCEL chain (Pydantic 구조화 출력) | 미사용 | LangChain 자체를 쓰지 않음 — Claude Code 서브에이전트(`.claude/agents/*.md`) 기반 오케스트레이션이라 체인/구조화 출력 스키마 계층이 없음 |
...
| 11 | Observability · Trace | 부분사용 | `review_log.md`(FAIL 판정 전문 누적)와 채팅 진행 매트릭스가 관찰 수단이지만, LangSmith류의 정식 트레이싱·스팬 로깅 도구는 없음 |
| 12 | 평가 (RAGAS·LLM-as-Judge) | 사용 | `reviewer` 서브에이전트가 형식(양식 준수)과 독자 흡인력을 기준으로 PASS/FAIL을 내리는 전형적인 **LLM-as-Judge** 패턴(RAGAS 같은 정량 지표 프레임워크는 아님) |

**요약**: 12개 중 **사용 6개**(2, 4, 7, 9, 10, 12) · **부분사용 3개**(6, 8, 11) · **미사용
3개**(1, 3, 5).
