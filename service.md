...
- **어떻게 돈을 벌 수 있는가**: 1차는 블로그 자체 수익(애드센스/애드핏 광고, 검색 유입 증가). 발행
  ...
  구독형 SaaS가 가능하다 — `backend/spike/`에서 Claude Agent SDK로 앵글 선택을 웹 UI로 치환하는
...
| LangChain Core (LCEL) + Pydantic | 구조화 | `prompt \| llm.with_structured_output(Schema).with_retry()` 파이프 | `backend/chains/`, `backend/rag/` |
...
세부 케이스는 `test_queries.csv`의 guardrail(18~20번)과 negative(9~12번) 항목에서 검증한다.

## 5. 성공 기준

- **인-아웃 케이스 통과율**: `test_queries.csv` 20건 중 **17건 이상(85%)** 통과를 완성 기준으로 한다.
